...
<body>
  ...
  <script>
    const formatInput = (id, mask) => {
      const el = document.getElementById(id);
      el.addEventListener('input', () => {
        let val = el.value.replace(/\D/g, ''), formatted = '';
        if (mask === 'card') {
        ...
        const start = el.selectionStart, delta = formatted.length - el.value.length;
        el.value = formatted;
        el.setSelectionRange(start + delta, start + delta);
      });
    };
    formatInput('card-number', 'card');
    formatInput('expiry-date', 'expiry');
