...
<body>
  ...
  <script>
    const mask = (el, pattern) => {
      const val = el.value.replace(/\D/g, ''), start = el.selectionStart;
      let res = '', j = 0;
      for (let i = 0; i < pattern.length && j < val.length; i++) {
        res += pattern[i] === '#' ? val[j++] : pattern[i];
      }
      const delta = res.length - el.value.length;
      el.value = res;
      el.setSelectionRange(start + delta, start + delta);
    };
    const cn = document.getElementById('card-number'), ed = document.getElementById('expiry-date');
    cn?.addEventListener('input', () => mask(cn, '#### #### #### ####'));
    ed?.addEventListener('input', () => mask(ed, '##/##'));
