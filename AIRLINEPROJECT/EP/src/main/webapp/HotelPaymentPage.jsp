  ...
  <script>
    ...
    const maskInput = (input, format) => {
      input.addEventListener('input', (e) => {
        ...
        const prevSeparators = (e.target.value.substring(0, start).match(/[\s/]/g) || []).length;
        e.target.value = formatted;
        const newSeparators = (formatted.substring(0, start).match(/[\s/]/g) || []).length;
        delta = newSeparators - prevSeparators;
        e.target.setSelectionRange(start + delta, start + delta);
      });
    };

    maskInput(cardInput, 'card');
    maskInput(expiryInput, 'expiry');
