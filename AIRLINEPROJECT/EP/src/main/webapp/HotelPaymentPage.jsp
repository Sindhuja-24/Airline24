...
<body>
  ...
  <script>
    const f = (e, m) => {
      let v = e.target.value.replace(/\D/g, ''), s = e.target.selectionStart, d = 0;
      if (m === 'card') {
        let nv = v.match(/.{1,4}/g)?.join(' ') || '';
        d = nv.length - e.target.value.length;
        e.target.value = nv;
      } else {
        let nv = v.match(/.{1,2}/g)?.join('/') || '';
        if (nv.length > 5) nv = nv.substring(0, 5);
        d = nv.length - e.target.value.length;
        e.target.value = nv;
      }
      e.target.setSelectionRange(s + d, s + d);
    };
    document.getElementById('card-number').addEventListener('input', (e) => f(e, 'card'));
    document.getElementById('expiry-date').addEventListener('input', (e) => f(e, 'expiry'));
