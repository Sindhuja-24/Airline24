...
<body>
  ...
  <script>
    const fmt = (el, fn) => el.addEventListener('input', () => {
      const c = el.selectionStart, v = el.value, f = fn(v);
      el.value = f;
      const d = f.length - v.length;
      if (d) el.setSelectionRange(c + d, c + d);
    });
    fmt(document.getElementById('card-number'), v => v.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim());
    fmt(document.getElementById('expiry-date'), v => {
      let r = v.replace(/\D/g, '');
      return r.length > 2 ? r.slice(0, 2) + '/' + r.slice(2, 4) : r;
    });
