...
<body>
  ...
  <script>
    ...
    const mask = (value, limit, separator) => {
      const regex = new RegExp(`(\\d{${limit}})(?=\\d)`, 'g');
      return value.replace(/[^\d]/g, '').replace(regex, `$1${separator}`).trim();
    };

    const handleInput = (e, limit, separator) => {
      const target = e.target;
      const start = target.selectionStart;
      const prevVal = target.value;
      const newVal = mask(target.value, limit, separator);

      if (prevVal !== newVal) {
        target.value = newVal;
        const delta = newVal.length - prevVal.length;
        target.setSelectionRange(start + delta, start + delta);
      }
    };

    cardInput.addEventListener('input', (e) => handleInput(e, 4, ' '));
    expiryInput.addEventListener('input', (e) => handleInput(e, 2, '/'));
