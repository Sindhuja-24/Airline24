...
<nav class="main-menu top-menu">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
      ...
      <div class="bg-blue-50 rounded-lg p-4">
        <p class="text-sm text-blue-800 flex items-center">
          <i data-lucide="shield-check" class="h-4 w-4 mr-1" aria-hidden="true"></i>
  ...
  const mask = (input, formatter, separator) => {
    input.addEventListener('input', (e) => {
      const prev = input.value;
      const cursor = input.selectionStart;
      const clean = prev.replace(new RegExp(`\\${separator}`, 'g'), '');
      const formatted = formatter(clean);
      const separatorsBefore = (prev.substring(0, cursor).match(new RegExp(`\\${separator}`, 'g')) || []).length;
      input.value = formatted;
      const separatorsAfter = (formatted.substring(0, cursor).match(new RegExp(`\\${separator}`, 'g')) || []).length;
      const newCursor = cursor + (separatorsAfter - separatorsBefore);
